...
| `SearchPhases` | Search phases by name and/or description |
...
| `CreatePhase` | Create a phase, optionally with its assemblies |
| `UpdatePhase` | Update a phase (only the provided fields change; provided items replace all current ones) |
| `DeletePhase` | Delete a phase and its assembly links (the assemblies themselves are kept) |
...
Phase items are objects: `{ "assemblyId": 1, "quantity": 2.5 }`.
